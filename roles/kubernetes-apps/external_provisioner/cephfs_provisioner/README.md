...
``` bash
docker run -ti -v /root/.kube:/kube -v /var/run/kubernetes:/var/run/kubernetes --privileged --net=host cephfs-provisioner /usr/local/bin/cephfs-provisioner -controlplane=http://127.0.0.1:8080 -kubeconfig=/kube/config -id=cephfs-provisioner-1
```
