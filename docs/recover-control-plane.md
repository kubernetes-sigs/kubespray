...
* Place the surviving nodes of the control plane first in the "etcd" and "kube-controlplane" groups
* Add the new nodes below the surviving control plane nodes in the "etcd" and "kube-controlplane" groups
...
* Move any broken controlplane nodes into the "broken\_kube-controlplane" group.

Then run the playbook with ```--limit etcd,kube-controlplane``` and increase the number of ETCD retries by setting ```-e etcd_retries=10``` or something even larger. The amount of retries required is difficult to predict.
