...
Update the inventory and run `cluster.yml` passing `--limit=etcd,kube_control_plane -e skip_etcd_even_nodes_check=yes`.
...
Run `upgrade-cluster.yml` also passing `--limit=etcd,kube_control_plane -e skip_etcd_even_nodes_check=yes`. This is necessary to update all etcd configuration in the cluster.
