#!/bin/bash -e
#
# This is an experimental script to rotate your control plane to a new set of nodes, while keeping
# the API and the control plane up.
#
# It expects:
...
do
    ...
    # needs to update : local LB to the apiserver + etcd config for network plugin such as calico
    INDEX=$index ansible-playbook $ANSIBLE_PLAYBOOKS_ARGS $KUBESPRAY_ROOT/playbooks/upgrade_cluster.yml --tags nginx,haproxy,etcd --limit kube_node
...
INDEX=$index ansible-playbook $ANSIBLE_PLAYBOOKS_ARGS $KUBESPRAY_ROOT/playbooks/cluster.yml --limit 'kube_control_plane,etcd'
# Update localhost loadbalancer control-plane on nodes
INDEX=$index ansible-playbook $ANSIBLE_PLAYBOOKS_ARGS $KUBESPRAY_ROOT/playbooks/upgrade_cluster.yml --tags nginx,haproxy,etcd --limit kube_node -e upgrade_cluster_setup=true
