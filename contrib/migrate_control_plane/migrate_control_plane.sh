...
INDEX=0 ansible -m debug -a var=node -e node='{{ groups.kube_control_plane[-1] }},{{ groups.etcd[-1]}}' kube_control_plane,etcd

# Add new control plane (last place), will be switched to first place in the first loop iteration
ROTATION=1 INDEX=1 ansible -m debug -a var=inventory_hostname kube_control_plane,etcd

for ((index=1;index<NUMBER_OF_CONTROL_PLANE;index++))
do
    # add new control plane / reorder
    INDEX=$index ansible -m debug -a var=inventory_hostname kube_control_plane,etcd
    # remove old control plane
    INDEX=$index ansible -m debug -a var=node -e node=node='{{ groups.kube_control_plane[-1] }},{{ groups.etcd[-1]}}' kube_control_plane,etcd
done
