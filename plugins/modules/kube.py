...
- name: test nginx is present
  kubernetes.kubespray.kube: name=nginx resource=rc state=present

- name: test nginx is stopped
  kubernetes.kubespray.kube: name=nginx resource=rc state=stopped

- name: test nginx is absent
  kubernetes.kubespray.kube: name=nginx resource=rc state=absent

- name: test nginx is present
  kubernetes.kubespray.kube: filename=/tmp/nginx.yml

- name: test nginx and postgresql are present
  kubernetes.kubespray.kube: files=/tmp/nginx.yml,/tmp/postgresql.yml

- name: test nginx and postgresql are present
  kubernetes.kubespray.kube:
    files:
      - /tmp/nginx.yml
      - /tmp/postgresql.yml
"""
